...
export class Migration20251113164235 extends Migration {
  ...
  override async up(): Promise<void> {
    ...
    this.addSql(
      `create index "otp_identifier_token_index" on "auth"."otp" ("identifier", "token");`,
    );

    this.addSql(
      `alter table "auth"."user" add column "is_phone_verified" boolean null default false, add column "user_type" varchar(255) not null;`,
    );
    this.addSql(
      `alter table "auth"."user" alter column "phone_number" type varchar(255) using ("phone_number"::varchar(255));`,
    );
    this.addSql(
      `alter table "auth"."user" alter column "phone_number" set not null;`,
    );
  }
